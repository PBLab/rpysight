...
will 'replace' that function with a Rust-equivalent function that also calls
the rendering pipeline, but the signature of that Rust function is the same as
the signature of their 'fast_process' function.
"""
...
""")
    ...
    with TimeTagger.SynchronizedMeasurements(tagger) as measurementGroup:
        ...
        if np.all(y_custom_histogram == y_histogram):
            print("The custom implementation shows the very same data as the Histogram from the API.")
        else:
            print("ERROR: The custom implementation has a different result than the Histogram from the API!")
